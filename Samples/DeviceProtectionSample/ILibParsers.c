...
int ILibGetLocalIPv4AddressList(struct sockaddr_in** addresslist, int includeloopback)
{
	...
	*addresslist = NULL;
	...
	if (WSAIoctl(sock, SIO_GET_INTERFACE_LIST, NULL, 0, &interfaces, 128 * sizeof(INTERFACE_INFO), &interfaces_count, NULL, NULL) == SOCKET_ERROR || interfaces_count == 0)
	{
		closesocket(sock);
...
void ILibChain_SafeRemoveSink(void *object)
{
	...
	{
		prev = chain;
		chain = (struct ILibBaseChain*)chain->Next;
	}
...
void ILibChain_SubChain_Destroy(void *object)
{
	...
	{
		temp = (struct ILibBaseChain*)baseChain->Next;
		free(baseChain);
		baseChain = temp;
	}
#ifdef WIN32
...
void ILibChain_SafeAdd_SubChain(void *chain, void *subChain)
{
	...
	while (newChain!=NULL && newChain->Object!=NULL)
	{
		...
		newChain = (struct ILibBaseChain*)newChain->Next;
	}
...
void ILibChain_DestroyEx(void *subChain)
{
	...
	while (newChain != NULL && newChain->Object != NULL)
	{
		...
		temp = (struct ILibBaseChain*)newChain->Next;
		free(newChain);
		newChain = temp;
	}
}
...
*/
void ILibParseUri(char* URI, char** Addr, unsigned short* Port, char** Path, struct sockaddr_in6* AddrStruct)
{
...
void ILibLifeTime_Check(void *LifeTimeMonitorObject, void *readset, void *writeset, void *errorset, int* blocktime)
{
	...
	//
	if (LifeTimeMonitor->NextTriggerTick >= CurrentTick)
	{
		if (LifeTimeMonitor->NextTriggerTick != (unsigned long)~0 && *blocktime > (int)(LifeTimeMonitor->NextTriggerTick - CurrentTick))
		{
			*blocktime = (int)(LifeTimeMonitor->NextTriggerTick - CurrentTick);
		}
		return;
	}
	LifeTimeMonitor->NextTriggerTick = (unsigned long)~0;
	...
	while (node != NULL)
	{
		...
		{
			if (Temp->ExpirationTick < LifeTimeMonitor->NextTriggerTick) LifeTimeMonitor->NextTriggerTick = Temp->ExpirationTick; // Save the next smallest value
			node = ILibLinkedList_GetNextNode(node);
		}
	}
...
{
	int len;
	if ((*str = malloc(256)) == NULL) ILIBCRITICALEXIT(254);
...
void ILibGetAddrFromBlob(char* ptr, int len, unsigned short port, struct sockaddr_in6 *addr)
{
	...
	else if (len == 16 || len == 20)
	{
		...
		if (len == 20) addr->sin6_scope_id = ((unsigned int*)(ptr + 16))[0];
	}
...
void ILibCriticalLog(char* msg, char* file, int line, int user1, int user2)
{
