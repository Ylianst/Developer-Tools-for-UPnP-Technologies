				...
				*/
				if (NewSocket != ~0)
				{
					...
					if (module->OnConnect != NULL)
					{
						...
						module->OnConnect(module, NULL, NULL, module->AsyncSockets[i], &(data->user));
					#else
...
void ILibAsyncServerSocket_OnConnectSink(ILibAsyncSocket_SocketModule socketModule, int Connected, void *user)
{
	...
	#ifndef MICROSTACK_NOTLS
	certChain = ILibAsyncSocket_SslGetCerts(socketModule); // vbl added this
	cert = ILibAsyncSocket_SslGetCert(socketModule);
	if (data->module->OnConnect != NULL) data->module->OnConnect(data->module, cert, certChain, socketModule, &(data->user));
	#else
...
ILibAsyncServerSocket_ServerModule ILibCreateAsyncServerSocketModule(void *Chain, int MaxConnections, unsigned short PortNumber, int initialBufferSize, int loopbackFlag, ILibAsyncServerSocket_OnConnect OnConnect, ILibAsyncServerSocket_OnDisconnect OnDisconnect, ILibAsyncServerSocket_OnReceive OnReceive, ILibAsyncServerSocket_OnInterrupt OnInterrupt, ILibAsyncServerSocket_OnSendOK OnSendOK)
{
	...
	if ((RetVal->ListenSocket = socket(localif.sin6_family, SOCK_STREAM, IPPROTO_TCP)) == -1) { return 0; }
	...
	// Bind the socket
#if defined(WIN32)
	if (bind(RetVal->ListenSocket, (struct sockaddr*)&localif, INET_SOCKADDR_LENGTH(localif.sin6_family)) != 0) { closesocket(RetVal->ListenSocket); return 0; }
#else
	if (bind(RetVal->ListenSocket, (struct sockaddr*)&localif, INET_SOCKADDR_LENGTH(localif.sin6_family)) != 0) { close(RetVal->ListenSocket); return 0; }
#endif
