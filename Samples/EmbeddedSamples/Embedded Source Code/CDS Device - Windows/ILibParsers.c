...
int ILibGetLocalIPAddressList(int** pp_int)
{
	...
	WSAIoctl(TempSocket,SIO_ADDRESS_LIST_QUERY,NULL,0,buffer,64,&bufferSize,NULL,NULL);
	
	*pp_int = (int*)MALLOC(sizeof(int)*((SOCKET_ADDRESS_LIST*)buffer)->iAddressCount);
	...
	return(((SOCKET_ADDRESS_LIST*)buffer)->iAddressCount);
}
