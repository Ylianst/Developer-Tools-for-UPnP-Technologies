...
void ILibAddToChain(void *Chain, void *object)
{
	...
	// put it on the front instead
	/*
	if(chain->Object != NULL)
	{
		struct ILibBaseChain *next = (struct ILibBaseChain*)MALLOC(sizeof(struct ILibBaseChain));
		next->Next = chain->Next;
		next->Object = chain->Object;
		next->Terminate = chain->Terminate;
		next->TerminateFlag = chain->TerminateFlag;
		chain->Next = next;
	}
	chain->Object = object;
	*/
