...
{
	struct MiniWebServerObject *mws = ((struct MiniWebServerObject*)object);
	int i;
	for(i=0;i<mws->MaxConnections;++i)
	{
		if(mws->Readers[i].Body!=NULL && mws->Readers[i].Body_MallocSize!=0)
		{
			FREE(mws->Readers[i].Body);
		}
		if(mws->Readers[i].PacketHeader!=NULL)
		{
			ILibDestructPacket(mws->Readers[i].PacketHeader);
		}
	}
...
void ILibMiniWebServerModule_PreSelect(void *WebServerModule,fd_set *readset, fd_set *writeset, fd_set *errorset,int *blocktime)
{
	...
	{
		module->PortNumber = ILibGetStreamSocket(htonl(INADDR_ANY),0,(HANDLE*)&(module->ListenSocket));
