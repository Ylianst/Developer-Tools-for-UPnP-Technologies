	...
	if(wsm->VirtualDirectoryTable!=NULL)
	{
		if(ws->Reserved7==NULL)
		{
			...
			if(ILibHasEntry(wsm->VirtualDirectoryTable,tmp,tmpLength)!=0)
			{
				...
				((ILibWebServer_VirtualDirectory)ws->Reserved7)(ws,header,bodyBuffer,beginPointer,endPointer,done);
			}
...
void ILibWebServer_RequestAnswered(struct ILibWebServer_Session *session)
{
	struct packetheader *hdr = ILibWebClient_GetHeaderFromDataObject(session->Reserved3);
	struct packetheader_field_node *f = hdr->FirstField;
...
void ILibWebServer_OnSendOK(void *AsyncServerSocketModule,void *ConnectionToken, void *user)
{
	...
	{
		ILibWebServer_RequestAnswered(session);
	}
	if(session->OnSendOK!=NULL)
	{
...
int ILibWebServer_StreamHeader_Raw(struct ILibWebServer_Session *session, int StatusCode,char *StatusData,char *ResponseHeaders, int ResponseHeaders_FREE)
{
	...
	ILibWebServer_Send_Raw(session,buffer,bufferLength,0,0);
	...
	{
		ILibWebServer_Send_Raw(session,ResponseHeaders,(int)strlen(ResponseHeaders),ResponseHeaders_FREE,0);
	}
...
int ILibWebServer_StreamBody(struct ILibWebServer_Session *session, char *buffer, int bufferSize, int userFree, int done)
{
	...
	else
	{
		...
		if(bufferSize>0)
		{
			...
			ILibWebServer_Send_Raw(session,hex,hexLen,0,0);
			ILibWebServer_Send_Raw(session,buffer,bufferSize,userFree,0);
...
int ILibWebServer_RegisterVirtualDirectory(void *WebServerToken, char *vd, int vdLength, ILibWebServer_VirtualDirectory OnVirtualDirectory)
{
	...
	{
		ILibAddEntry(s->VirtualDirectoryTable,vd,vdLength,(void*)OnVirtualDirectory);
	}
	return(0);
}
