...
const char *UPNPCP_SOAP_Header = "POST %s HTTP/1.0\r\nHost: %s:%d\r\nUser-Agent: WINDOWS, UPnP/1.0, Intel MicroStack/1.0.1181\r\nSOAPACTION: \"%s#%s\"\r\nContent-Type: text/xml\r\nContent-Length: %d\r\n\r\n";
...
void UPnPParseUri(char* URI, char** IP, unsigned short* Port, char** Path)
{
...
struct packetheader *UPnPBuildPacket(char* IP, int Port, char* Path, char* cmd)
{
	...
	ILibAddHeaderLine(RetVal,"User-Agent",10,"WINDOWS, UPnP/1.0, Intel MicroStack/1.0.1181",44);
	FREE(HostLine);
	return(RetVal);
}
...
void UPnPExpireRequests(struct UPnPDevice *device)
{
	...
	while(ed!=NULL)
	{
		UPnPExpireRequests(ed);
		ed = ed->Next;
	}
	while(s!=NULL)
	{
		ILibDeleteRequests(((struct UPnPCP*)s->Parent->CP)->HTTP,s);
		s = s->Next;
	}
}
...
void UPnPSCPD_Sink(void *reader, struct packetheader *header, char* buffer, int *p_BeginPointer, int EndPointer, int done, void* dv, void* sv)
{
	struct UPnPDevice *device;
...
void UPnPSCPD_Fetch(struct UPnPDevice *device)
{
	...
	while(s!=NULL)
	{
		UPnPParseUri(s->SCPDURL,&IP,&Port,&Path);
...
void UPnPProcessDeviceXML(void *v_CP,char* buffer, int BufferSize, char* LocationURL, int RecvAddr, int Timeout)
{
	...
	if(BaseURL==NULL)
	{
		UPnPParseUri(LocationURL,&IP,&Port,&Path);
...
void UPnPHTTP_Sink_DeviceDescription(void *reader, struct packetheader *header, char* buffer, int *p_BeginPointer, int EndPointer, int done, void* user, void* cp)
{
...
void UPnPOnEventSink(void *ReaderObject, struct packetheader *header, char* buffer, int *BeginPointer, int BufferSize, int done, void* user)
{
	...
	if(done!=0)
	{
		...
		ILibAddHeaderLine(resp,"Server",6,"WINDOWS, UPnP/1.0, Intel MicroStack/1.0.1181",44);
...
void UPnPOnSubscribeSink(void *ReaderObject, struct packetheader *header, char* buffer, int *BeginPointer, int BufferSize, int done, void* user, void *vcp)
{
	struct UPnPService *s;
...
void UPnPRenew(void *state)
{
	...
	UPnPParseUri(service->SubscriptionURL,&IP,&Port,&Path);
	...
	ILibAddHeaderLine(p,"User-Agent",10,"WINDOWS, UPnP/1.0, Intel MicroStack/1.0.1181",44);
...
void *UPnPCreateControlPoint(void *Chain, void(*A)(struct UPnPDevice*),void(*R)(struct UPnPDevice*))
{
	...
	cp->SSDP = ILibCreateSSDPClientModule(Chain,"upnp:rootdevice", 15, &UPnPSSDP_Sink,cp);
